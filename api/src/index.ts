...
io.use(async (socket, next) => {
  try {
    ...
    const clientColor = socket.handshake.auth.color;
    ...
    if (token) {
      ...
      userId = user.id;
      userName = clientName || user.name || user.email;
      ...
      userName = clientName || 'Anonymous User';
    } else {
      return next(new Error('Authentication required'));
    }
    
    socket.data = { userId, boardId, pageId, canEdit, userName, color: clientColor || '#3b82f6' };
...
io.on('connection', (socket) => {
  ...
  const handleShapeEvent = async (eventName: string, data: any) => {
    const { boardId, pageId } = data;
  ...
  socket.on('presence:cursor', (data) => {
    ...
    if (!pageId) return; // Skip if no page context
