    ...
    try {
        ...
        if (shareToken) {
            ...
            try {
                ...
                const membership = board.memberships.find((m) => m.userId === authReq.user.id);
                const membershipRole = membership?.role;
                canEdit = isOwner || membershipRole === 'OWNER' || membershipRole === 'EDITOR';
            }
            catch (authError) {
        ...
        // Store events
        const storedEvents = await Promise.all(events.map((event) => prisma.boardEvent.create({
            data: {
                boardId: id,
                type: event.type,
                payload: event
...
router.post('/:id/pages', authenticateToken, async (req, res, next) => {
    try {
        ...
        const membership = board.memberships.find((m) => m.userId === req.user.id);
        const membershipRole = membership?.role;
...
router.put('/:id/pages/:pageId', authenticateToken, async (req, res, next) => {
    try {
        ...
        if (parsedData.width !== undefined)
            updateData.width = parsedData.width;
        if (parsedData.height !== undefined)
            updateData.height = parsedData.height;
        if (parsedData.backgroundType !== undefined)
            updateData.backgroundType = parsedData.backgroundType;
        if (parsedData.gridType !== undefined)
            updateData.gridType = parsedData.gridType;
        if (parsedData.gridSize !== undefined)
            updateData.gridSize = parsedData.gridSize;
        if (parsedData.showAxes !== undefined)
            updateData.showAxes = parsedData.showAxes;
        if (parsedData.assetId !== undefined)
            updateData.assetId = parsedData.assetId;
        if (parsedData.pdfPage !== undefined)
            updateData.pdfPage = parsedData.pdfPage;
...
router.post('/:id/pages/reorder', authenticateToken, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        // Update indices
        await Promise.all(order.map((pageId, index) => prisma.boardPage.update({
            where: { id: pageId },
            data: { index }
        })));
...
router.delete('/:id/pages/:pageId', authenticateToken, async (req, res, next) => {
    try {
        const { id, pageId } = req.params;
        // Check permissions
        const board = await prisma.board.findUnique({
